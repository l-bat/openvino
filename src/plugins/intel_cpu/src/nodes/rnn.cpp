...
void RNN::fillCellDesc() {
    ...
    const auto recurrentDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(1));
    const auto cellStateDataType = haveCellState(cell_type) ?
        DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(2)) : memory::data_type::undef;
    const auto weightsDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(wIdx));
    const auto recurrentWeightsDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(rIdx));
    const auto biasDataType = memory::data_type::f32;
    const auto recurrentOutDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalOutputPrecisionAtPort(1));
    const auto outDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalOutputPrecisionAtPort(0));

    assert(recurrentDataType == recurrentOutDataType);
    ...
    const Shape outShape  = MemoryDescUtils::makeDummyShape({{T.minVal, N.minVal, SC}, {T.maxVal, N.maxVal, SC}});
    ...
    outDataDescs.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(outShape, outDataType, memory::format_tag::tnc));

    inDataDescs.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, recurrentDataType, memory::format_tag::ldnc));
    outDataDescs.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, recurrentOutDataType, memory::format_tag::ldnc));

    if (haveCellState(cell_type)) {
        inDataDescs.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, cellStateDataType, memory::format_tag::ldnc));
        outDataDescs.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, cellStateDataType, memory::format_tag::ldnc));
    }
    ...
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS, recurrentDataType, memory::format_tag::nc));
    outCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS, outDataType, memory::format_tag::nc));

    if (haveCellState(cell_type)) {
        inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS, cellStateDataType, memory::format_tag::nc));
        outCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeS, cellStateDataType, memory::format_tag::nc));
    }

    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(WShape, weightsDataType, memory::format_tag::nc));
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(RShape, recurrentWeightsDataType, memory::format_tag::nc));
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(BShape, biasDataType, memory::format_tag::x));
...
void RNN::fillSequenceDesc() {
    ...
    const auto cellStateDataType = haveCellState(cell_type) ?
        DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(2)) : memory::data_type::undef;
    const auto sequenceDataType = memory::data_type::s32;
    const auto weightsDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(wIdx));
    const auto recurrentWeightsDataType = DnnlExtensionUtils::IEPrecisionToDataType(getOriginalInputPrecisionAtPort(rIdx));
    const auto biasDataType = memory::data_type::f32;
    ...
    if (nativeOrder) {
        ...
        dstLayerMemoryFormat = D == 2 ? memory::format_tag::abcd : memory::format_tag::tnc;
    } else if (N.isStatic() && N.maxVal == 1) { // WA to avoid reorder before sequence for some models. @todo "D" is declared as a constant ?!?!
        srcLayerMemoryFormat = memory::format_tag::tnc;
    ...
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNTDC, dataType, srcLayerMemoryFormat));
    outCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNTSC, outDataType, dstLayerMemoryFormat));

    auto srcIterMemoryFormat = memory::format_tag::undef;
    auto dstIterMemoryFormat = memory::format_tag::undef;

    srcIterMemoryFormat = memory::format_tag::tnc;
    dstIterMemoryFormat = memory::format_tag::tnc;

    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNDSC, recurrentDataType, srcIterMemoryFormat));
    outCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNDSC, recurrentOutDataType, dstIterMemoryFormat));

    // initial cell state
    if (haveCellState(cell_type)) {
        auto srcIterCellMemoryFormat = memory::format_tag::undef;
        auto dstIterCellMemoryFormat = memory::format_tag::undef;

        srcIterCellMemoryFormat = memory::format_tag::tnc;
        dstIterCellMemoryFormat = memory::format_tag::tnc;

        inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNDSC, cellStateDataType, srcIterCellMemoryFormat));
        outCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(shapeNDSC, cellStateDataType, dstIterCellMemoryFormat));
    }

    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(TShape, sequenceDataType, memory::format_tag::x)); // sequence lengths
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(WShape, weightsDataType, memory::format_tag::ntc)); // W
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(RShape, recurrentWeightsDataType, memory::format_tag::ntc)); // R
    inCandidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(BShape, biasDataType, memory::format_tag::nc)); // B
...
void RNN::fillWeights(const int *gate_map, const size_t wIdx, const size_t rIdx) {
    ...
    const auto& weightPrec = dataPrecision == Precision::BF16 ? dataPrecision : getOriginalInputPrecisionAtPort(wIdx);
    if (!verifyWeightsPrecision(dataPrecision, weightPrec)) {
    ...
    TensorDesc w_data_desc(weightPrec, dims_w, getWeightsLayoutByDims(dims_w, false));
    ...
    TensorDesc w_state_desc(weightPrec, dims_s, getWeightsLayoutByDims(dims_s, false));
    ...
    // todo: convert is needed only for bf16?
    // std::cout << "Convert from " << weightPrec.name() << " to " << dataPrecision.name() << "\n";
...
void RNN::createDescriptor(const std::vector<MemoryDescPtr> &inputDesc,
    ...
    if (descs.empty()) {
        ...
        wDescs[0] = dnnl::memory::desc(weightsDims, weightsDataType, wFormat);
        auto statesDims = DnnlExtensionUtils::convertToDnnlDims(VectorDims{ L, D, SC, G, SC });
        wDescs[1] = dnnl::memory::desc(statesDims, recurrentWeightsDataType, wFormat);
...
void RNN::prepareParams() {
    ...
    assert(dataType        == recurrentDataType);
    assert(weightsDataType == recurrentWeightsDataType);
    ...
    outDataDescs[0] = std::make_shared<DnnlBlockedMemoryDesc>(Shape{SL, B, D * SC}, outDataType, memory::format_tag::tnc);

    inDataDescs[1] = std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, recurrentDataType, memory::format_tag::ldnc);
    outDataDescs[1] = std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, recurrentOutDataType, memory::format_tag::ldnc); //

    if (haveCellState(cell_type)) {
        inDataDescs[2] = std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, cellStateDataType, memory::format_tag::ldnc);
        outDataDescs[2] = std::make_shared<DnnlBlockedMemoryDesc>(shapeS_4D, cellStateDataType, memory::format_tag::ldnc);
    }

    bool wFormatWasChanged = false;
    // WA To avoid different weights layer and iter formats in FP32 case.
    if (dataType == memory::data_type::f32 && (SL != 1 || B < optimalBatchSize)) {
    ...
    if (!wasMemoryPrepared || wFormatWasChanged) {
        auto itpd = descs[0].createPrimitiveDescriptorIterator(getEngine(), *attr);
        prepareMemory(itpd);
